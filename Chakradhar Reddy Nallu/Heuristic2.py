...
def heuristic_2(kitchen_items=[], goal_node=None):
    if(len(foon_object_to_FU_map[goal_node.id])>=0):
        ...
        while len(items_to_search) > 0:
            ...
            if not check_if_exist_in_kitchen(kitchen_items, current_item):
                ...
                if len(candidate_units)>1 :
                    min=len(foon_functional_units[candidate_units[0]].input_nodes)
                    selected_candidate_idx=candidate_units[0]
                    for i in candidate_units:
                        k=foon_functional_units[i].input_nodes
                        if len(k)<min:
                            min=len(k)
    ...
    for node in goal_nodes:
        ...
        for object in foon_object_nodes:
            if object.check_object_equal(node_object):
                output_task_tree = heuristic_2(kitchen_items, object)
