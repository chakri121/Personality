...
def IDFS_algorithm(kitchen_items=[], goal_node=None):
    max_depth=20
                ...
                for node in foon_functional_units[
                    ...
                    if node_idx not in items_to_search:
                        ...
                        if flag:                         
                            ...
                            #print(child_nodes)                      #append items_to_search in middle to a variable child nodes
                if(len(child_nodes)!=0 and dict_depth.get(current_item_index)==j):
                    flag1+=1
                
                    continue

                for kk in child_nodes:
                    dict_depth[kk]=dict_depth.get(current_item_index)+1                             
                dumy_items=items_to_search
    
                items_to_search=child_nodes+dumy_items
    ...
    for node in goal_nodes:
        ...
        for object in foon_object_nodes:
            if object.check_object_equal(node_object):
                output_task_tree = IDFS_algorithm(kitchen_items, object)
                save_paths_to_file(output_task_tree,
                                   'output_IDFS_{}.txt'.format(node["label"]))
                no_goal_exist=[1]
                break
        if(no_goal_exist[0]==0):
