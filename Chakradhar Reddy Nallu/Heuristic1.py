...
def heuristic1(kitchen_items=[], goal_node=None):
    ...
    while len(items_to_search) > 0:
        ...
        if not check_if_exist_in_kitchen(kitchen_items, current_item):
            ...
            # this is the part where you should use heuristic for Greedy Best-First search
             # Sort candidates based on inputs
            ...
            if len(candidate_units)!=0 :
                ...
                for i in candidate_units:
                    motion_check=foon_functional_units[i].motion_node
                    if maximum<float(dict_motion[motion_check]):
                        maximum=float(dict_motion[motion_check])
...
def read_universal_foon(filepath='FOON.pkl'):
   ...
   # print(functional_units[0].motion_node)
